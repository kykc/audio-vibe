...
This is a **greenfield rewrite with no code yet** — the only tracked file is `design_doc.md`
(922 lines, status "accepted (stack + toolchain), implementation not started").
...
re-litigated. Section references below (§n) point into it.
...
**The client is rewritten first (§1.3, §7.3).** The existing unmodified APO stays deployed; the
new client attaches as a protocol v1 valet. All APO-side decisions (§8) are deferred and do not
block client work.

## Hard rules

### 1. Protocol v1 is frozen (§4)

§4 is a normative wire specification, not a description of intent. The new client must
interoperate with the *existing* APO binary. Deviations are defects, not improvements —
improvements go to §9.1 (a future v2). Specifically:

- Object names, the 1 MiB mapping, and both events being **manual-reset** are fixed (§4.2).
- The payload is **planar/de-interleaved**, shared in place: sample `s` of channel `c` at float
  index `c * (size / channelCount) + s`. `size` is a total sample count, not a frame count (§4.3).
...
- Dead legacy that is **not** part of v1 and must not be reintroduced: `MAIN_SIZE`/`MAIN_NAME`,
  ...
  `ThreadSafeContainer` (§4.2, §7.4.4).

### 2. The audio thread is real-time safe, without exception (§7.4)

This is a merge gate, not an aspiration. Read §7.4 in full before touching `protocol/`, `ipc/`,
...
- **Forbidden on the audio thread:** any heap activity (`new`/`delete`, `std::vector`/`string`
  ...
  throwing exceptions; unbounded loops; first-touch page faults (§7.4.1).
- **The one sanctioned exception** is exhaustive: `SetEvent`/`ResetEvent`/`WaitForSingleObject`
  ...
  rendezvous *is* the protocol (§7.4.4).
- Preallocate at stream open and touch every page. Control plane ↔ audio thread communication
  goes through lock-free SPSC queues with bounded drain per block (§7.4.2).
- **Plugin chain mutation never happens on the audio thread.** Build the chain on the control
  ...
  upstream — that is a design defect (§7.4.3).
- **Acceptance criterion (§7.4.3):** user-initiated transitions *may* click — that is acceptable
...
- Our VST3 host interface implementations are on this call stack too.
  ...
  thread* — it must enqueue lock-free and return (§7.4.5).
- `RealtimeSanitizer` is unavailable under MSVC. The substitute is a debug-only violation
  ...
  primitives. Compiled into `RelWithDebInfo`, out of release (§7.4.6).

## Planned structure (§7.1)
...
short-lived scanner process per scan (§7.2).

## Stack (§5, §6) — decided, verified end to end
...
and web UIs are disqualified (§5.1, §5.4). Embedding uses `QWindow::fromWinId()` →
...
and `git config --system core.longpaths true` (§6.2).
...
and the one all verification in §10 used (§6.4).

### Build traps — each was hit and resolved already (§6.3)
...
5. **The SDK's CMake is not a well-behaved dependency.** `SMTG_ENABLE_VST3_HOSTING_EXAMPLES=OFF`
   ...
   66 targets. Tolerable initially; §9.6 is to replace it with an in-house static library over
...
blank. Use `PrintWindow`/`BitBlt` if a plugin editor ever needs capturing (§6.5).

## Testing

The protocol conformance harness is a **first-class deliverable**, replacing the predecessor's
manual interactive `DebugStream` (§4.7). It needs a synthetic king (drives blocks at
...
Plus the §7.4.3 soak test: flat audio-thread allocation count (exactly zero) and flat RSS over a
long idle run.
...
- The APO blocks the audio engine's real-time thread for up to **1000 ms** waiting on the valet.
  ...
  the reason §9.1 exists (§3.7.1).
- All shared objects use a **null DACL** — any process on the machine can open them, steal the
  stream, inject audio, or wedge `audiodg.exe` (§3.7.2, §9.2).
- Registry ownership of endpoint keys is taken and never restored (§3.7.5, §9.4).

## Open items (§11)

1. Pin the VST3 SDK archive `URL_HASH` — blocks the first build.
2. Minimum OS floor (§8.1, leaning Windows 11 only) — project owner, blocks APO stage.
3. Independently verify GFX vs modern registration slots (§8.2, leaning GFX `,2` only, since the
