// A chain preset: the rack on its own, in a file the user names and keeps.
//
...
// ask. What it holds is the rack exactly as the session file holds it, key for key
// (`src/rack_yaml.h`), and nothing else -- no window geometry, no endpoint, and above all no scan
// catalog, which is this machine's inventory rather than anything about the chain.
//
...
/// False and a filled `error` on anything not understood, with `rack` left untouched: unreadable,
/// malformed YAML, a version this build does not know, no `rack` sequence, an entry that names no
/// plugin, or a state blob that will not decode. See the note above for why this is stricter than
/// `readSession`.
///
...
[[nodiscard]] bool writePreset(const std::filesystem::path& path,
                               const std::vector<RackEntry>& rack, std::string& error);
