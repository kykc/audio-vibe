...
This document is for a **developer working on the project**. End-user documentation lives in
`README.md`; the installer is out of scope -- and there is no installer: the product ships as a
zip of the portable folder, installed from the shell itself (sec. 6.8).
...
action -- that node has no Node.js, so even the checkout is hand-rolled git.
